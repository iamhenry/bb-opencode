...
- [x] Mobile: PWA/desktop can show the Agent chip. The **iOS native** home compose dock (`Plan, ask, build…`) never runs plugin frontend — no in-composer chip is possible. iOS picks the agent via Settings → OpenCode → Default OpenCode agent (`select`: build / plan / orchestrator).
